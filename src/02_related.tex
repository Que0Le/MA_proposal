...
% DPDK (Data Plane Development Kit) is a library created by Intel's engineer Venky Venkatesan and is being maintained by Linux foundation, that consists of libraries to accelerate packet processing workloads. The library effectively dismouns the NIC and CPU cores from the kernel to built an very efficient and high througput data path. The library has been used in numerous project, namely pfSense, Open vSwitch/OpenStack, Intel's DDP and in 5G UPF implementations. Recently, DPDK teams proposed a QoS plugin enhancement on GTP-U packet stream level for UPF
...
% This reduce the task handle managing and monitoring for the \ac{NIC}s
